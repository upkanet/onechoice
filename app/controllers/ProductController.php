...
class ProductController extends BaseController{
	...
	public function search($room){
		...
		$products = $products->where('isActive','=',1)->where('products.name','LIKE','%'.$q.'%')->select('products.name as pName','products.permalink as pLink','products.img_path as pImg','categories.name as cName','categories.permalink as cLink')->groupby('products.id')->get();
